...
{
    public class LeadJustDial
    {
        public class LeadJustDialCommand : IRequest<BaseDto>
        {
        ...
        public class Handler : IRequestHandler<LeadJustDialCommand, BaseDto>
        {
            ...
            public async Task<BaseDto> Handle(LeadJustDialCommand request, CancellationToken cancellationToken)
            {
                //handler logic goes here
                var justDialLead = _mapper.Map<LeadJustDialCommand, JustDialLead>(request);

                var dbLead = _context.Leads
                                .Where(x => x.LeadSourceId == justDialLead.LeadId
                                    && x.LeadSource == "Just Dial")
                                .FirstOrDefault();

                var lead = _mapper.Map<LeadJustDialCommand, Lead>(request);
                ...
                await _context.JustDialLeads.AddAsync(justDialLead);
